...
<template>
  <div class="min-h-dvh flex flex-col bg-[#F8FAFC]">
    <main class="flex-1 flex flex-col mx-auto w-full max-w-md px-5 pt-8 pb-10">
      <div
        class="mb-4 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-[13px] text-amber-950"
        role="note"
      >
      ...
      <button
        type="button"
        class="mb-6 self-start text-[14px] font-medium text-blue-700 hover:text-blue-900"
        @click="goBack"
      >
        ← กลับ
      </button>
      ...
      <template v-else>
        <h1 class="text-[20px] font-bold text-slate-900 mb-1">จำลองเข้าสู่ระบบ ThaiD</h1>
        ...
        <div
          ...
          class="mb-5 rounded-xl border border-slate-200 bg-white px-4 py-3 text-[13px] text-slate-700 shadow-sm"
        >
