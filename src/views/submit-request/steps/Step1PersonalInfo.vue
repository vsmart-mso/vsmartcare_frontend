...
function isValidMobile(val: string): boolean {
  // เบอร์มือถือไทย: 10 หลัก ขึ้นต้นด้วย 0
  return /^0[0-9]{9}$/.test(val.trim())
}
...
const errors = computed(() => ({
  ...
  mobile:
    touched.mobile && !isValidMobile(mobile.value)
      ? 'เบอร์มือถือต้อง 10 หลัก ขึ้นต้นด้วย 0'
      : '',
...
function handleRentInput(e: Event) {
  ...
  displayRent.value  = digits   // sync displayRent ระหว่างพิมพ์ด้วย
  el.value = digits
}

// ค่าเช่า: แสดงตัวเลขล้วนเมื่อ focus เพื่อให้แก้ไขง่าย
function handleRentFocus(e: Event) {
  touched.rentPerMonth = true
  const el = e.target as HTMLInputElement
  el.value = rentPerMonth.value
  displayRent.value = rentPerMonth.value
}

// ค่าเช่า: ใส่ลูกน้ำเมื่อ blur เช่น 1000 → "1,000"
function handleRentBlur() {
  if (!rentPerMonth.value) { displayRent.value = ''; return }
  const n = parseInt(rentPerMonth.value, 10)
  if (!isNaN(n)) displayRent.value = n.toLocaleString('th-TH')
}
  ...
  <div class="space-y-4">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      <div class="p-4">
        ...
        <!-- โทรศัพท์ (บ้าน/ที่ทำงาน) — optional -->
        <div class="mb-3">
