  ...
  <div v-else class="space-y-4">
    ...
    <div v-if="!props.filterFields" class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      <div class="flex items-center gap-3 bg-blue-50 px-4 py-3 border-b border-blue-100">
        ...
        <p class="text-[14px] font-bold text-[#1A56DB]">ผู้ประสบปัญหาทางสังคม</p>
      </div>
      <div class="p-4">
    ...
    <div v-if="show('family_members_count')" class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      <div class="flex items-center gap-3 bg-blue-50 px-4 py-3 border-b border-blue-100">
        ...
        <p class="text-[14px] font-bold text-[#1A56DB]">สมาชิกในครัวเรือน</p>
      </div>
      <div class="p-4">
        <div class="flex items-center gap-2 mb-3">
          <span class="bg-blue-100 text-[#1A56DB] text-[11px] font-bold px-2 py-0.5 rounded-md">5.1</span>
          <span class="text-[13px] font-medium text-slate-600">รายชื่อสมาชิกในครัวเรือน</span>
            ...
            <div class="pl-4 pr-3 pt-3 pb-3">
              ...
              <div class="flex items-start gap-2">
                ...
                <div class="w-6 h-6 rounded-full bg-blue-100 flex items-center justify-center flex-shrink-0 mt-0.5">
                  <span class="text-[11px] font-bold text-[#1A56DB]">{{ m.seq }}</span>
                </div>
              ...
              <div v-if="m.occupation || m.monthlyIncome || m.physicalCondition !== 'normal' || !m.selfCare"
                ...
                <span v-if="m.occupation"
                  class="inline-flex items-center gap-1 bg-slate-100 text-slate-600 text-[11px] px-2 py-0.5 rounded-full">
                ...
                <span v-if="m.monthlyIncome"
                  class="inline-flex items-center gap-1 bg-emerald-50 text-emerald-700 text-[11px] px-2 py-0.5 rounded-full">
                  <svg class="w-3 h-3 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor">
                ...
                <span v-if="!m.selfCare"
                  class="inline-flex items-center gap-1 bg-red-50 text-red-600 text-[11px] px-2 py-0.5 rounded-full">
                  <svg class="w-3 h-3 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor">
        ...
        <div v-if="editingMember !== null" class="mt-3 border border-slate-200 rounded-xl p-4 bg-white shadow-sm">
            ...
            <div class="col-span-2">
              ...
              <div class="flex gap-2">
                ...
                <select
                  ...
                  class="w-[62px] shrink-0 bg-white border rounded-xl px-2 py-2.5 text-[13px] focus:outline-none focus:ring-2 transition-colors"
                ...
                <select
                  ...
                  @change="handleDobPartChange"
                  class="flex-1 min-w-0 bg-white border rounded-xl px-3 py-2.5 text-[13px] focus:outline-none focus:ring-2 transition-colors"
                  :class="editMemberErrors.dateOfBirth ? 'border-red-300 focus:ring-red-200' : 'border-slate-200 focus:ring-[#1A56DB]/30 focus:border-[#1A56DB]'">
                ...
                <select
                  ...
                  @change="handleDobPartChange"
                  class="w-[88px] shrink-0 bg-white border rounded-xl px-2 py-2.5 text-[13px] focus:outline-none focus:ring-2 transition-colors"
                  :class="editMemberErrors.dateOfBirth ? 'border-red-300 focus:ring-red-200' : 'border-slate-200 focus:ring-[#1A56DB]/30 focus:border-[#1A56DB]'">
                  <option value="">ปี พ.ศ.</option>
                  <option v-for="y in dobYearOptions" :key="y" :value="String(y)">{{ y }}</option>
                </select>
              </div>
              <p v-if="editMemberErrors.dateOfBirth" class="text-[11px] text-red-500 mt-1">{{ editMemberErrors.dateOfBirth }}</p>
              <p v-else-if="editingMember.dateOfBirth" class="text-[11px] text-slate-500 mt-1">
          ...
          <div class="flex gap-2 mt-4 justify-end">
            <button type="button" @click="cancelEditMember"
              class="px-4 py-1.5 text-[13px] border border-slate-300 rounded-xl text-slate-600 hover:bg-slate-100 transition-colors">
              ยกเลิก
            </button>
            <button type="button" @click="saveMember"
              class="px-4 py-1.5 text-[13px] bg-[#1A56DB] text-white rounded-xl hover:bg-blue-700 transition-colors">
              บันทึก
            </button>
          </div>
        </div>
